...
async function callGroqOnce({ apiKey, model, messages, responseJsonSchema, maxOutputTokens, reasoningEffort }) {
  ...
  if (!res.ok) {
    ...
    // Il tool_call rifiutato include comunque il contenuto generato in
    // error.failed_generation — lo recuperiamo invece di considerare il
    // tentativo fallito. Due casi distinti visti in produzione:
    // 1) tool_call con "name" vuoto -> failed_generation è JSON {name,arguments}
    // 2) il modello scrive testo libero invece di chiamare il tool (tipico
    //    quando fa una domanda di chiarimento) -> failed_generation è prosa,
    //    non JSON. In questo caso lo trattiamo come "reply": tutti gli schemi
    //    attuali hanno un campo reply, o falliscono in modo controllato se
    //    assente (es. LIFESTYLE_SCHEMA -> nessun "lifestyle" -> gestito lato
    //    frontend come errore, nessun crash).
    if (responseJsonSchema) {
      try {
        ...
        if (failedGen) {
          try {
            ...
            if (typeof failedGen === 'string' && failedGen.trim()) {
              return { ok: true, text: JSON.stringify({ reply: failedGen.trim() }) };
            }
          }
        }
      } catch {
