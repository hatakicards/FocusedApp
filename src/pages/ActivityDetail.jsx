...
import ActivityDetailDesktop from '@/components/desktop/ActivityDetailDesktop';
import {
  AlertDialog,
  AlertDialogContent,
  AlertDialogHeader,
...
export default function ActivityDetail() {
  ...
  const aRatings = useMemo(
    () => (ratings || []).filter((r) => r.activity_id === id),
    [ratings, id]
  );

  const aGoals = useMemo(
    () => (goals || []).filter((g) => g.activity_id === id),
    [goals, id]
  );
  ...
  if (activity && !isMobile) {
    return (
      <>
        <ActivityDetailDesktop activity={activity} aRatings={aRatings} aGoals={aGoals} />
        <TutorialDialog pageId="activity_detail" />
      </>
    );
  }
  ...
  const handleDeleteGoal = (goalId) => {
    optimisticDeleteGoal(goalId).catch(console.error);
  };
  ...
  return (
    <div className="px-5 safe-top pb-4">
      ...
      {activity.preset_type === 'phone_time' && (
        <PhoneTimeSection activityId={activity.id} />
      )}

      {activity.preset_type === 'reading' && (
        <ReadingSection activityId={activity.id} />
      )}

      {activity.preset_type === 'quit_smoking' && (
        <QuitSmokingSection activityId={activity.id} />
      )}

      {activity.is_gym && (
        <button
          onClick={() => sub.isPro ? setScheduleOpen(true) : setShowPremium(true)}
          className="mb-8 w-full rounded-2xl border border-border bg-card p-4 flex items-center justify-between hover:bg-accent transition-colors"
        >
          <span className="text-sm font-semibold">{t('detail_scheda')}</span>
          {sub.isPro ? (
            <span className="text-xs text-muted-foreground">{t('modifica')}</span>
          ) : (
              ...
              return (
                ...
                >
                  <button
                    ...
                    style={{
                      borderColor: completedNow ? diff.color : 'hsl(0 0% 30%)',
                      backgroundColor: completedNow ? diff.color : 'transparent',
                    }}
                  >
                    {completedNow && <Check size={14} className="text-background" strokeWidth={3} />}
                  </button>
                  <div className="flex-1 min-w-0">
                    <span className={`text-sm font-medium ${completedNow ? 'line-through text-muted-foreground' : ''}`}>
                      {goal.title}
                    </span>
      ...
      <TutorialDialog pageId="activitydetail" />
    </div>
  );
}